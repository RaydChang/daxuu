...
public class DBTable {
    ...
    public String getKeyName()
    {
            ...
            if (key.attributes("n").Value == "false")
            {
                ret = key.Name;
            }else
            {
	...
	public  int getDatabaseType()
    {
        get
        {
            int ret = (int)dbType.sqlserver;
            String provider = Manuan.eBridge.Framework.Data.Global.GetProvider(this.DatabaseName);
            switch (provider)
            {
                case "System.Data.SQLite":
                    ret = (int)dbType.sqlite;
                    break;
                case "System.Data.SqlClient":
                    ret = (int)dbType.sqlserver;
                    break;
                case "System.Data.OracleClient":
                    ret = (int)dbType.oracle;
                    break;
    ...
    public String GetFields(String argFieldName,String argKeyValue)
    {
        ...
        String sqlCond = String.Format("{0} = '{1}'", new String[] { this.KeyName,this.KeyValue }); 

        XmlDocModel xData = DALHelper.Query(this.DatabaseName, this.TableName, "*",
            sqlCond, false);
        if (xData != null && xData.ChildNodes[1].FirstChild.ChildNodes.Count == 1)
        {
            xnFld = xData.SelectSingleNode("/xconfig/records/record/" + argFieldName.ToLower());
        ...
        {
            throw new Framework.MyException.PFException("没有取到正确资料，可能是主键值:[{0}]不正确，有且只有一笔。表：{1}，主键：{2}", new String[] { this.KeyValue, this.TableName, argFieldName });
        }
        return ret;
    ...
    {
        DataSet ret = null;
        String sql="",dbName=this.DatabaseName;
    ...
    {
        return Query(this.DatabaseName, s_dsn, xs_fields, xs_conds);
    }
    ...
    public  XmlDocModel QueryXml(String s_fields, String s_cond)
    {
        ...
        DataSet ds = Query(s_fields, s_cond);
        ret = Tools.DataSetToXML(ds);

        return ret;
    }

    public  XmlDocModel QueryXml(String argDbName, String argDataSourceName, String argSql)
    {
        XmlDocModel ret = null;
    ...
    public  String QueryJson(String s_condorsql)
    {
        ...
        DataSet ds = Query(s_condorsql);
        //ret = Tools.DataSetToXML(ds);
        //Helper.Tools.XmlToJSON(xData.GetData(start, end, this.PosXslPath), info);
    ...
    {
        return AddRow(this.DatabaseName, argTableName, argFieldList, argValueList);
    }
    /// <summary>
    /// 新增一资料行，默认当前TABLE
    /// </summary>
    ...
    {
        return this.AddRow(this.TableName, argFieldList, argValueList);
    }
    ...
    /// <param name="argValueList">值清单</param>
    /// <returns>成功新增行数</returns>
    public  int AddRow(XmlNode argData, XmlNode argConf)
    {
    ...
    /// <param name="argValueList">值清单</param>
    /// <returns>成功新增行数</returns>
    public  int AddRow(XmlNode argData)
    {
        ...
        foreach (XmlNode xRow in this.TableSchema.ChildNodes[1].FirstChild.ChildNodes)
        {
            ...
            XmlNode xdCol = argData.SelectSingleNode(xRow.Name.ToLower());
        ...
        return this.AddRow(this.TableName, sbFields.ToString(), sbValues.ToString());
    }
    /// <summary>
    /// 新增一资料行，默认当前TABLE
    /// </summary>
    ...
    {
        return DeleteRow(this.DatabaseName, argTableName, argWhere, argIsKey);
    }
    /// <summary>
    /// 删除一资料行，依KEY栏
    /// </summary>
    ...
    {
        return this.DeleteRow(this.TableName, argWhere, argKey);
    }
    ...
    public  int DeleteRow(String argFieldName, String argFieldValue)
    {
        ...
        return this.DeleteRow(this.TableName, where, true);
    }
    ...
    {
        return UpdateRow(this.DatabaseName, argTableName, argSet, argWhere);
    }
    ...
    {
        return this.UpdateRow(this.TableName, argSet, argWhere);
    }
    ...
    public  int UpdateRow(XmlNode argData)
    {
        ...
        return this.UpdateRow(this.TableName, sbSet.ToString(), sbWhere.ToString());
    }
