 ...
 * 类描述：  
 * @author lucky  
 * 创建时间：Jun 13, 2013 10:04:15 AM  
 * 修改人：lucky
 * 修改时间：Jun 13, 2013 10:04:15 AM  
 * 修改备注：  
 * @version  
 *
 */
public class DALHelper {
	 ...
	 * @param dbName :数据库名称，在app.cfg中设定，注意设定的标签全部使用小写
	 * @return Connection对象
	 * @author lucky  
	 * 创建时间：Jun 13, 2013 10:31:12 AM  
	 * 修改人：lucky
	 * 修改时间：Jun 13, 2013 10:31:12 AM  
	 * 修改备注：  
	 * @version  
	 *
	 */
     ...
     * 
     * 功能概述：关闭Database链接  
     * @param conn Database连接对象
     * @return 是否成功关闭
     * @author lucky  
     * 创建时间：Jun 13, 2013 3:27:48 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 3:27:48 PM  
     * 修改备注：  
     * @version  
     *
     */
     ...
     * @return 是否成功关闭
     * @author lucky  
     * 创建时间：Jun 13, 2013 3:30:03 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 3:30:03 PM  
     * 修改备注：  
     * @version  
     *
     */
    ...
    //判斷是否為Primary Key欄
    private static boolean isPrimaryKey(ResultSet rsKeys, String fldName) throws SQLException {
        boolean ret = false;
        //int i =1;
	 ...
	 * @author lucky  
	 * 创建时间：Jun 13, 2013 10:10:43 AM  
	 * 修改人：lucky
	 * 修改时间：Jun 13, 2013 10:10:43 AM  
	 * 修改备注：  
	 * @version  
	 *
	 */
    public static XmlDocModel GetSchema(String argDatabaseName, String argDataSourceName)
    {
        String path = Global.AppDir() + "/data/schema/" + argDataSourceName.toLowerCase() + ".xml";
     ...
     * @param dsn table或view名称
     * @param dbName 数据库名称
     * @return table或view的结构結構
     * @author lucky  
     * 创建时间：Jun 13, 2013 2:18:47 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 2:18:47 PM  
     * 修改备注：建立的Schema将保存到硬盘，文件名称为小写  
     * @version  
     * @throws SQLException 
     *
     */
    public static XmlDocModel CreateSchema(String dsn, String dbName)
    {
        ...
        xSchema.Save(doc, Global.AppDir()+"/data/schema/" + dsn.toLowerCase() + ".xml");
     ...
     * @throws SQLException
     * @author lucky  
     * 创建时间：Jun 13, 2013 10:54:24 AM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 10:54:24 AM  
     * 修改备注：从XmlDomModel类中移过来
     * @version  
     *
     */
...
/*
 * XmlDocModel中有Save方法，直接使用
    /// <summary>
    /// 保存XML文檔到提定目的地
    /// </summary>
    /// <param name="argXml">待保存的XMLDocument Object</param>
    /// <param name="argFileName">文件名稱，可含Path</param>
    public static void Save(XmlDocModel argXml, String argFileName)
    {
        String path = "";
        String dir = "";
        String file = "";
        String destFile = "";//保存目的地

        String fileName = argFileName.Replace("/", "\\");//實現不區分“/”與“\”

        int posPath = fileName.LastIndexOf("\\");//最後一"\"
        if (posPath < 0) posPath = 0;
        ...
        else
        {
            dir = AppDomain.CurrentDomain.SetupInformation.ApplicationBase +
                ((path.IndexOf("\\") == 0) ? "\\" : "") + path;
            destFile = dir + ((file.IndexOf("\\") == 0) ? "\\" : "") + file;
        }
        //若有指定目錄
        if (!path.Equals(""))
        {
        ...
        try
        {
            ...
            argXml.Save(destFile);
        }
        catch (System.Exception e)
        {
            throw new Manuan.eBridge.Framework.MyException.PFException("PF0010", e.Message);
        }
...
*/
     ...
     * 功能概述：执行SQL语句，返回成功笔数  
     * @param argSql SQL语句
     * @param argDbName Dababase
     * @return 成功笔数 
     * @throws DALException
     * @author lucky  
     * 创建时间：Jun 13, 2013 3:50:10 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 3:50:10 PM  
     * 修改备注：  
     * @version  
     *
     */
    public static int ExecuteNonQuery(String argSql,String argDbName)
    {
        ...
        catch (Exception ex)
        {
            ...
            int pos1 = argSql.toLowerCase().indexOf("insert into ");
            if (pos1 < 0)
            {
                pos1 = argSql.toLowerCase().indexOf("update ");
            }
            int pos2 = argSql.toLowerCase().indexOf(" ", pos1 + 6);
            //處理FROM后連續多個空格現象，如from    project
            //while (argSql.Substring(pos2, 1)==" ")
            //{
            //    pos2 = argSql.ToLower().IndexOf(" ", pos2+1);
            //}
            if (pos1 > 0 && pos2 > 0)
            {
                dsn = argSql.substring(pos1 + 6, pos2 - pos1);
            }
            */
            DALException exDb =
                new DALException("DB01", "", ex, argSql);
     ...
     * @param argSql SQL语句
     * @param argDbName Database名称
     * @return 查找到的结果集合
     * @author lucky  
     * 创建时间：Jun 13, 2013 4:27:55 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 4:27:55 PM  
     * 修改备注：  
     * @version  
     *
     */
    public static XmlDocModel Query(String argSql,String argDbName)
    {
        ...
        catch (Exception ex)
        {
            ...
            String dsn = "";
            //select * FROM  PROJECT WHERE....
            int pos1 = argSql.ToLower().IndexOf("from ");
            int pos2 = argSql.ToLower().IndexOf(" ", pos1 + 6);
            //處理FROM后連續多個空格現象，如from    project
            //while (argSql.Substring(pos2, 1)==" ")
            //{
            //    pos2 = argSql.ToLower().IndexOf(" ", pos2+1);
     ...
     * @param argDbName Database名称
     * @param argDataSourceName table or view名
     * @param argFields 字段式，多个用逗号分隔
     * @param argCond 条件表达式，不能带WHERE,可以带ORDER BY ，如：LastName = 'Rose' AND Birday = '2000/09/01'
     * @return 查找到的结果集合
     * @author lucky  
     * 创建时间：Jun 13, 2013 4:30:34 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 4:30:34 PM  
     * 修改备注：  
     * @version  
     *
     */
    public static XmlDocModel Query(String argDbName, String argDataSourceName, String argFields, String argCond)
    {
    ...
    /*
    /// <summary>
    /// 取得資料，以JSON格式返回，指定DB，TABLE，Field,條件，封裝類型
    /// </summary>
    /// <param name="argDbName">资料庫名称</param>
    /// <param name="argDataSourceName">资料来源名称，如TABLE、View名称</param>
    /// <param name="argFields">栏位清单，以","号分隔</param>
    /// <param name="argCond">条件表达式，如：LastName = 'Rose' AND Birday = '2000/09/01'</param>
    /// <param name="argIsFlat">將欄位的節點變成屬性</param>
    /// <param name="argIsText">true:若单独取一个栏位，且只有一笔资料时直接取得返回；false:將结果封装成XML</param>
    /// <returns>将资料以文字形体返回.</returns>
    /// <remarks>
    /// <para>[規格說明]
    /// 指定DB，TABLE，Field,條件，封裝類型
    /// </para>
    ...
    ///	</example>
    /// </remarks>
    public static String Query(String argDbName, String argDataSourceName, String argFields, String argCond)
    {
        String ret = null;
        ...
        catch (Exception ex)
        {
            MyException.DbAccessException exDb =
                new MyException.DbAccessException("DB00", argDataSourceName, ex, sql);
            throw exDb;
        }
        finally
        {
            ...
            {
                Manuan.eBridge.Framework.MyLogger.Write(sql, "", "Message", argDataSourceName);
            }
        }
     ...
     * @return
     * @author lucky  
     * 创建时间：Jun 13, 2013 4:54:54 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 4:54:54 PM  
     * 修改备注：  
     * @version  
     *
     */
    public static ResultSet QueryResultSet(String argSql)
    {
    	return QueryResultSet(argSql,"_DEFAULT_DATABASE");
    }
     ...
     * @return 符合条件的数据包对象
     * @author lucky  
     * 创建时间：Jun 13, 2013 4:45:33 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 4:45:33 PM  
     * 修改备注：  
     * @version  
     *
     */
    public static ResultSet QueryResultSet(String argSql,String argDbName)
    {
        Connection db = null;
        Statement st = null;
        ResultSet rs = null;
     ...
     * @param argDbName：DB名
     * @param argDataSourceName：table或view名
     * @param argFields：字段表达式
     * @param argCond 条件表达式
     * @return 
     * @author lucky  
     * 创建时间：Jun 13, 2013 4:50:40 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 4:50:40 PM  
     * 修改备注：  
     * @version  
     *
     */
    public static ResultSet QueryResultSet(String argDbName, String argDataSourceName, String argFields, String argCond)
    {

        String sql = "SELECT %1$s FROM %2$s WHERE %3$s";
        sql = String.format(sql, argFields, argDataSourceName, argCond);
        return QueryResultSet(sql,argDbName);
    }
    ...
    ///	</example>
    /// </remarks>
    public static XmlDocModel Query(String argDbName, String argDataSourceName, String argSql, bool argIsFlat)
    {
        XmlDocModel ret = null;
        ...
        try
        {
            using (IDataReader dataReader = db.ExecuteReader(dbCommand))
            {
                ...
                ret = Data.Convert2.DataReaderToXml(dataReader, argIsFlat);
    ...
    /// <param name="argIsFlat">將欄位的節點變成屬性</param>
    /// <returns></returns>
    public static XmlDocModel Query(String argDbName, String argSql, bool argIsFlat)
    {
        XmlDocModel ret = null;
        ...
        {
            MyException.DbAccessException exDb =
                new MyException.DbAccessException("DB00", argDbName, ex, sql);
            throw exDb;
        }
        finally
        {
            ...
            if (Manuan.eBridge.Framework.Data.Global.AppConfig("open_log") == "1")
            {
                Manuan.eBridge.Framework.MyLogger.Write(sql, "", "Message", argDbName);
            }
        }
	...
	*/
     ...
     * @return
     * @author lucky  
     * 创建时间：Jun 13, 2013 5:28:07 PM  
     * 修改人：lucky
     * 修改时间：Jun 13, 2013 5:28:07 PM  
     * 修改备注：  
     * @version  
     *
     */
     ...
     *
     */
    public static int AddRow(String argTableName, String argFieldList, String argValueList)
    {
        return AddRow(argTableName,argFieldList,argValueList);
...
/*
    ...
    public static int DeleteRow(String argDbName, String argTableName, String argWhere, boolean argIsKey)
    {
        ...
        {
            MyException.DbAccessException exDb =
                new MyException.DbAccessException("DB03", argDbName + "." + argTableName, ex, sql);
            throw exDb;
        }
        finally
        {
            ...
            if (Manuan.eBridge.Framework.Data.Global.AppConfig("open_log") == "1")
            {
                Manuan.eBridge.Framework.MyLogger.Write(sql, "", "Message", argTableName);
            }
        }
    ...
    public static int UpdateRow(String argDbName, String argTableName, String argSet, String argWhere)
    {
        ...
        {
            MyException.DbAccessException exDb =
                new MyException.DbAccessException("DB02", argDbName+"."+argTableName, ex, sql);
            throw exDb;
        }
        finally
        {
    ...
    public static String GetValue(String argDbName, String argDSN, String argField, String argCond)
    {
        String ret = null;
        XmlDocModel xData = Query(argDbName, argDSN, argField, argCond, false);
        if (!xData.IsEmpty)
        {
            ret = xData.ChildNodes[1].FirstChild.FirstChild.FirstChild.InnerText;
        }
        return ret;
    }
    ...
    /// <param name="argCond">條件</param>
    /// <returns>欄值，當沒有找到，返回null</returns>
    public static String GetValue(String argDbName, String argDSN, String argSql)
    {
        String ret = null;
        XmlDocModel xData = Query(argDbName, argDSN, argSql, false);
        if (!xData.IsEmpty)
        {
            ret = xData.ChildNodes[1].FirstChild.FirstChild.FirstChild.InnerText;
        }
    ...
    /// <param name="argCond">條件</param>
    /// <returns>欄值，當沒有找到，返回null</returns>
    public static String GetValue(String argDbName, String argDSN, String argSql,bool isall)
    {
        String ret = null;
        XmlDocModel xData = Query(argDbName, argDSN, argSql, false);
        if (!xData.IsEmpty)
    ...
    /// 取得單一欄值
    /// </summary>
    /// <param name="argFields">欄名稱,可多個，以逗號（，）分隔</param>
    /// <param name="argCond">條件</param>
    /// <returns>欄值，當沒有找到，返回null</returns>
    public static StringDictionary GetValues(String argDbName, String argDSN, String argFields, String argCond)
    {
        StringDictionary ret = new StringDictionary();
        XmlDocModel xData = Query(argDbName, argDSN, argFields, argCond, false);
        String[] fields = argFields.Split(',');
        if (!xData.IsEmpty)
        {
            foreach (String fld in fields)
            {
                if (xData.ChildNodes[1].FirstChild.FirstChild.SelectSingleNode(fld) != null)
                {
                    ret.Add(fld, xData.ChildNodes[1].FirstChild.FirstChild.SelectSingleNode(fld).InnerText);
                }
            }
        }
        return ret;
    ...
    /// <param name="argCond">條件</param>
    /// <returns>欄值，當沒有找到，返回null</returns>
    public static String GetMaxValue(String argDbName, String argDSN, String argField, String argCond)
    {
        String ret = null;
        String fld = String.Format("max({0}) as maxvalue", new String[] { argField });
        XmlDocModel xData = Query(argDbName, argDSN, fld, argCond, false);
        if (!xData.IsEmpty)
        {
            ret = xData.ChildNodes[1].FirstChild.FirstChild.InnerText;
        }
        return ret;
    }
    /// <summary>
    ...
    /// <param name="argCond">條件</param>
    /// <returns>欄值，當沒有找到，返回null</returns>
    public static String GetMaxValue(String argDbName, String argDSN, String argField, String argCondName,
        String argCondValue)
    {
        String cond = String.Format("{0} = '{1}'", new String[] { argCondName, argCondValue });
