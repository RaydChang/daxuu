 ...
 * 类描述：编码器  
 * @author rayd  
 * 创建时间：Jun 7, 2013 2:44:44 PM  
 * 修改人：rayd
 * 修改时间：Jun 7, 2013 2:44:44 PM  
 * 修改备注：  
 * @version  
 *
 */
public class NOInfo {
	 ...
	 * 
	 * 功能概述：  
	 * @param s_filepath 編碼規則的XML文件路徑
	 * @param i_count 批量取得，
	 * @return 编码值
	 * @author rayd  
	 * 创建时间：Jun 7, 2013 2:56:57 PM  
	 * 修改人：rayd
	 * 修改时间：Jun 7, 2013 2:56:57 PM  
	 * 修改备注：  
	 * @version  
	 *
	 */
    public String MakeCode(String s_filepath, int i_count)
    {
        ...
        for (Iterator<?> it = root.elementIterator(); it.hasNext();) {
            ...
            if (kind.equalsIgnoreCase("$date"))
            {
                    ...
                    int currentValue = Integer.parseInt(el.attribute("value").getValue());
                    ...
                    if (el.attribute("format") != null)
                    {
                        DecimalFormat df = new DecimalFormat(el.attribute("format").getValue());
                        sbCode.append(df.format(this.nextValue));
                    }
                    else
                    {
                        sbCode.append(Integer.toString(this.nextValue));
                    }
